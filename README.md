# The New SDLC with Vibe Coding — 简体中文翻译

## 📖 阅读译文

👉 **[点击查看完整简体中文翻译](Day_1_v3_zh.md)**

## 文档简介

本仓库包含 Google 技术白皮书 **"The New SDLC with Vibe Coding"** 的简体中文翻译。

| 属性 | 内容 |
|------|------|
| 原标题 | The New SDLC with Vibe Coding |
| 作者 | Addy Osmani, Shubham Saboo, Sokratis Kartakis |
| 日期 | 2026年5月 |
| 篇幅 | 51页 / 约64,000字符 |
| 主题 | AI 如何重塑软件开发生命周期（SDLC），涵盖 Vibe Coding、Agentic Engineering、Context Engineering、工厂模型、开发者角色演变等 |

## 原文链接

- [Kaggle: The New SDLC with Vibe Coding](https://www.kaggle.com/whitepaper-the-new-SDLC-with-vibe-coding)

## 翻译说明

- 专业术语采用业界通用译法，首次出现时在括号内保留英文原词
- 人名、产品名、公司名保留英文
- 语气保持专业、权威、对技术读者友好
- 每页开头插入 `[Page X]` 标记，便于与原文对照

## 文件结构

```
.
├── Day_1_v3_zh.md    # 简体中文翻译（Markdown）
├── Day_1_v3.pdf      # 原始 PDF
└── README.md         # 本文件
```

## 版权说明

本文为翻译作品，原文版权归原作者所有。翻译仅用于学习和研究目的，请勿用于商业用途。

## 贡献

欢迎提交 Issue 或 Pull Request 改进翻译质量。

---

*翻译日期：2026年9月9日*
